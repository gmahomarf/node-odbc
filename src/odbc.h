...
#define REQ_ARGS(N)                                                     \
  if (args.Length() < (N))                                              \
    return NanThrowError(Exception::TypeError(                         \
                                               NanNew<String>("Expected " #N "arguments")));

//Require String Argument; Save String as Utf8
#define REQ_STR_ARG(I, VAR)                                             \
  if (args.Length() <= (I) || !args[I]->IsString())                     \
    return NanThrowError(Exception::TypeError(                         \
                                               NanNew<String>("Argument " #I " must be a string"))); \
...
#define REQ_STR_OR_NULL_ARG(I, VAR)                                             \
  if ( args.Length() <= (I) || (!args[I]->IsString() && !args[I]->IsNull()) )                     \
    return NanThrowError(Exception::TypeError(                         \
                                               NanNew<String>("Argument " #I " must be a string or null"))); \
  String::Utf8Value VAR(args[I]->ToString());
...
#define REQ_FUN_ARG(I, VAR)                                             \
  if (args.Length() <= (I) || !args[I]->IsFunction())                   \
    return NanThrowError(Exception::TypeError(                         \
                                               NanNew<String>("Argument " #I " must be a function"))); \
  Local<Function> VAR = Local<Function>::Cast(args[I]);

#define REQ_BOOL_ARG(I, VAR)                                             \
  if (args.Length() <= (I) || !args[I]->IsBoolean())                   \
    return NanThrowError(Exception::TypeError(                         \
                                               NanNew<String>("Argument " #I " must be a boolean"))); \
  Local<Boolean> VAR = (args[I]->ToBoolean());
  
#define REQ_EXT_ARG(I, VAR)                                             \
  if (args.Length() <= (I) || !args[I]->IsExternal())                   \
    return NanThrowError(Exception::TypeError(                         \
                                               NanNew<String>("Argument " #I " invalid"))); \
...
#define OPT_INT_ARG(I, VAR, DEFAULT)                                    \
  ...
  if (args.Length() <= (I)) {                                           \
    ...
    return NanThrowError(Exception::TypeError(                         \
                                               NanNew<String>("Argument " #I " must be an integer"))); \
  }
